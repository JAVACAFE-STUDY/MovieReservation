...
// 테스트 데이터는 프로파일 "withDummyData"이 있으며 embedded database환경에서만 실행
@Configuration
...
public class DummyDataCreator {
    ...
    public void initData(){
        getMovieCreateRequests().values().stream()
                .forEach(x->movieService.createMovie(x));

        getTheaterCreateRequests().values().stream()
                .forEach(x->theaterService.createTheater(x));
    }
